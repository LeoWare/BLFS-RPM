...
_prepare() {
	...
	if [ ! -e ${LOGPATH}/${1} ]; then
		...
		msg_line "	Prepare: Create dierctories: "
			[ -d ${LOGPATH} ]	|| install -dm 755 ${LOGPATH}
			[ -d ${INFOPATH} ]	|| install -dm 755 ${INFOPATH}
			[ -d ${SPECPATH} ]	|| install -dm 755 ${SPECPATH}
			[ -d ${PROVIDESPATH} ]	|| install -dm 755 ${PROVIDESPATH}
			[ -d ${REQUIRESPATH} ]	|| install -dm 755 ${REQUIRESPATH}
		msg_success
	fi		
	touch ${_log}
	return
